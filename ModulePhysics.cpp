...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius, ColliderType ctype, b2BodyType type, float restituition)

{
	...
	fixture.restitution = restituition;
...
PhysBody* ModulePhysics::CreateRectangleSensor(int x, int y, int width, int height, ColliderType type)
{
	b2BodyDef body;
	body.type = b2_staticBody;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
...
PhysBody* ModulePhysics::CreateChain(int x, int y, int* points, int size, b2BodyType type, float restituition)
{
	b2BodyDef body;
	body.type = type;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
